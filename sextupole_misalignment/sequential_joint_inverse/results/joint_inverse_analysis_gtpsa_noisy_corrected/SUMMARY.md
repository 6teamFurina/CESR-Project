...
zero-offset closed orbit with the paired zero-offset SciBmad/GTPSA periodic closed-orbit Jacobian, including the fixed BPM and corrector gains and 103 normal
steering controls. The stored reference and every current correction readback use independent means of 3,072 reads from the same `5.0 um` per-read BPM white-noise model (`0.090 um` standard deviation per mean). That baseline command is then held fixed throughout all
...
| 50 um/plane drift after noisy-BPM GTPSA baseline correction | physics_gls | 34.181 | 54.796 | 86.779 | 60.674 | 86.76% |
| 50 um/plane drift after noisy-BPM GTPSA baseline correction | shared_target_local_ridge | 33.477 | 53.641 | 83.026 | 60.016 | 87.31% |
| 50 um/plane drift after noisy-BPM GTPSA baseline correction | shared_joint_ridge | 33.458 | 53.535 | 83.360 | 59.850 | 87.27% |
| 50 um/plane drift after noisy-BPM GTPSA baseline correction | shared_joint_random_feature | 33.416 | 53.531 | 84.382 | 59.714 | 87.57% |
...
50-um/plane drift after noisy-BPM GTPSA baseline correction, the best learned result is
`shared_joint_random_feature` at
`33.416 um` RMSE.  Training
