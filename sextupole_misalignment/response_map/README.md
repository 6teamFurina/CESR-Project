...
```powershell
julia --project=. sextupole_misalignment/response_map/run_sextupole_alignment_gtpsa.jl `
  --start=1 --stop=19 --part-label=1 `
  --output-dir=sextupole_misalignment/response_map/results/full
```

Validate one magnet against exact four-corner numerical differences with:

```powershell
julia --project=. sextupole_misalignment/response_map/validate_sextupole_alignment_gtpsa.jl
```
...
```powershell
python sextupole_misalignment/response_map/aggregate_results.py
```
...
```powershell
python sextupole_misalignment/response_map/analyze_local_response_svd.py
```
