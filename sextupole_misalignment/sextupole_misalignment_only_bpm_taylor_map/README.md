...
```powershell
julia --project=. sextupole_misalignment/sextupole_misalignment_only_bpm_taylor_map/generate_only_sextupole_scans.jl `
  --targets=all `
  --output-dir=sextupole_misalignment/sextupole_misalignment_only_bpm_taylor_map/results/exact_scans
```

Run `analyze_inverses.py` in the validated WSL `bmad` environment, then run:

```powershell
python sextupole_misalignment/sextupole_misalignment_only_bpm_taylor_map/validate_results.py
```
