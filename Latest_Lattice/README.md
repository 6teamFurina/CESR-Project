...
This directory contains the archived CHESS-U 6 GeV CESR lattice and the first
reproducible Bmad-to-SciBmad conversion audit.

The concise acceptance table is in `FINAL_VALIDATION_SUMMARY.md`; the detailed
...
The two input files are exact copies of the files that were found in
`CESR Project/Archive`. No cryptographic hashes are calculated in this
project.

## Raw exports

- `latest_cesr_scibmad_bmad_20260814.jl`: authoritative raw export produced
  by Tao `20260814-0`, the latest conda-forge Bmad build found on 2026-08-16.
- `tao_write_scibmad_bmad_20260814.log`: complete latest-export log.
- `latest_cesr_scibmad.jl` and `tao_write_scibmad.log`: retained comparison
...
Tao> write scibmad latest_cesr_scibmad_bmad_20260814.jl
Tao> quit
```
...
& "C:/Users/JoeyN/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/python.exe" `
  "CESR Project/Latest_Lattice/build_repaired_lattice.py"
julia --startup-file=no --project="CESR Project" `
  "CESR Project/Latest_Lattice/load_repaired_lattice.jl"
```

## Numerical validation

- `LOCAL_MAP_COMPARISON.md` and `scibmad_local_map_comparison.csv` contain the
...
- `FULL_CONTROL_VALIDATION.md` compares one-pass tracking derivatives for all
...
- `GIRDER_VALIDATION.md` covers all 12 girders, all six alignment parameters,
...
- `PHOTON_BRANCH_VALIDATION.md` verifies exact element counts and lengths for
...
wsl.exe -d Ubuntu-Bmad -- `
  ...
  "/mnt/d/Ring_Design_Development/CESR Project/Latest_Lattice/diagnose_conversion.py"
```

This writes:

- `bmad_branches.csv`;
- `bmad_element_inventory.csv`;
- `bmad_control_lords.csv`;
- `bmad_control_relations.csv`;
- `conversion_inventory.json`.

The raw SciBmad load smoke test is:

```powershell
julia --startup-file=no --project="CESR Project" `
  "CESR Project/Latest_Lattice/load_raw_export.jl"
```

Its captured failure is in `scibmad_raw_load.log`.
