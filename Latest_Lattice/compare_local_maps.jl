...
const HERE = @__DIR__
const REFERENCE = joinpath(HERE, "bmad_reference_branch0", "local_maps.csv")
const OUTPUT = joinpath(HERE, "scibmad_local_map_comparison.csv")
const SUMMARY = joinpath(HERE, "LOCAL_MAP_COMPARISON.md")

include(joinpath(HERE, "latest_cesr_scibmad_repaired.jl"))
...
function main(args=ARGS)
    ...
    open(SUMMARY, "w") do io
        ...
        println(io, "The three `ID_S1A` slices must not be interpreted independently: the " *
                    ...
                    "across the complete five-element block. Run `compare_wiggler_block.jl` " *
