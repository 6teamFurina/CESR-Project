...
Run from the CESR project environment with:
    ...
    julia --project=. older_ring_version/wigglers/experiment_zero_orbit.jl

The experiment distinguishes transverse closure from the longitudinal path-
length term and inspects whether the two wigglers in `older_ring_version/cesr.jl` are active in
the default lattice.
"""
