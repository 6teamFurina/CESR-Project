...
```powershell
julia --project=. dataset_benchmark/optics/sextupole_alignment_gtpsa/response_map/run_sextupole_alignment_gtpsa.jl `
  --start=1 --stop=19 --part-label=1 `
  --output-dir=dataset_benchmark/optics/sextupole_alignment_gtpsa/response_map/results/full
```

Validate one magnet against exact four-corner numerical differences with:

```powershell
julia --project=. dataset_benchmark/optics/sextupole_alignment_gtpsa/response_map/validate_sextupole_alignment_gtpsa.jl
```
...
```powershell
python dataset_benchmark/optics/sextupole_alignment_gtpsa/response_map/aggregate_results.py
```
...
```powershell
python dataset_benchmark/optics/sextupole_alignment_gtpsa/response_map/analyze_local_response_svd.py
```
